...
from run_llava_gen import eval_model
import random
...
with open("edit_instructions.json", 'r') as file:
...
sdxl_device = "cuda:1"
# flux_device = "cuda:1"
# pixsigma_device = "cuda:2"
# pixalpha_device = "cuda:2"
# pgv2_device = "cuda:2"
# pgv25_device = "cuda:2"
# sd3_device = "cuda:2"
...
# org_model_id = "stabilityai/stable-diffusion-xl-base-1.0"
# org_pipe = StableDiffusionXLPipeline.from_pretrained(org_model_id, torch_dtype=weight_dtype, variant="fp16", use_safetensors=True).to(sdxl_device)
...
def generate_images(text_prompt):
    ...
    # seed = 309
        ...
        # org_image = org_pipe(text_prompt,generator=torch.Generator().manual_seed(seed), num_inference_steps=50, guidance_scale=sdxl_org_guidance_scale).images[0]

        all_images = []
        # for i, begin_image in enumerate([dpo_image, org_image]):
        for i, begin_image in enumerate([dpo_image]):
            ...
            for _ in range(sdxl_outer_loop_num):
                ...
                # inner_loop_state = inner_loop_state.to(device=sdxl_device, dtype=weight_dtype)
